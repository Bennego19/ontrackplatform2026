...
router.get("/",bruteforce.prevent, async (req,res) =>{
    let collection = await db.collection("internship");
...
//creating a cohort
router.post("/createinternship",bruteforce.prevent, async(req,res) =>{
    let newDocument ={
...
router.patch("/:id",bruteforce.prevent, async (req, res) => {
    ...
    let collection = db.collection("internship");
...
res.send(result).status(200);
  ...
  router.delete("/:id", bruteforce.prevent, async(req,res) =>{
        ...
        const collection = db.collection("internship");
